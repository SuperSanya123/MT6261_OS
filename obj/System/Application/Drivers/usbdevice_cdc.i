# 1 "Source\\Application\\Drivers\\usbdevice_cdc.c"
# 1 "C:\\Users\\user\\Desktop\\DZ09-master//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "Source\\Application\\Drivers\\usbdevice_cdc.c"
# 21 "Source\\Application\\Drivers\\usbdevice_cdc.c"
...
# 22 "Source\\Application\\Drivers\\usbdevice_cdc.c" 2
...
# 23 "Source\\Application\\Drivers\\usbdevice_cdc.c" 2
# 1 "Source\\Application\\Drivers\\usbdevice_cdc.h" 1
# 24 "Source\\Application\\Drivers\\usbdevice_cdc.c" 2
# 48 "Source\\Application\\Drivers\\usbdevice_cdc.c"
...
const struct sd00_cdc{ uint8_t Length; uint8_t DescriptorType; uint16_t String[(sizeof(0x0409) - 2) / 2];}sd00_cdc = {sizeof(sd00_cdc), 0x03, 0x0409};;
const struct sd01_cdc{ uint8_t Length; uint8_t DescriptorType; uint16_t String[(sizeof(u"Prolific Technology Inc. ") - 2) / 2];}sd01_cdc = {sizeof(sd01_cdc), 0x03, u"Prolific Technology Inc. "};;
const struct sd02_cdc{ uint8_t Length; uint8_t DescriptorType; uint16_t String[(sizeof(u"USB-Serial Controller D") - 2) / 2];}sd02_cdc = {sizeof(sd02_cdc), 0x03, u"USB-Serial Controller D"};;
...
static const TUSB_CFG_DESCR CFG_DESC_CDC =
{
    ...
    0x64,
    {
        ...
        7,
        0x05,
        (((USB_EP3IN) < USB_EP1OUT) ? (USB_EP3IN) : (USB_EP3IN) - USB_EP1OUT + USB_EP1IN) | USB_DIR_IN,
        0x03,
        16, 0x00,
        0x01,
        ...
        7,
        0x05,
        (((USB_EP1IN) < USB_EP1OUT) ? (USB_EP1IN) : (USB_EP1IN) - USB_EP1OUT + USB_EP1IN) | USB_DIR_IN,
        0x02,
        64, 0x00,
        0x00,
        ...
        7,
        0x05,
        (((USB_EP1OUT) < USB_EP1OUT) ? (USB_EP1OUT) : (USB_EP1OUT) - USB_EP1OUT + USB_EP1IN) | USB_DIR_OUT,
        0x02,
        64, 0x00,
        0x00
    }
...
static uint8_t CDC_OUTBuffer[64];
...
{
    return (Index < USB_CDC_GetStringDescriptorCount()) ? STR_DATA_CDC[Index] : 
# 177 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                                                                               ((void *)0)
# 177 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                                                                                   ;
}
...
static void USB_CDC_IntFlashTXBuffer(void)
{
    ...
    {
        USB_PrepareDataTransmit(USB_EP1IN, 
# 195 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                                                  ((void *)0)
# 195 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                                                      , 0);
...
static void USB_CDC_SetConnectedStatus(boolean Connected)
{
    if (USB_CDC_Connected != Connected)
    {
        ...
        if ((IntEventerInfo != 
# 230 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                              ((void *)0)
# 230 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                                  ) && (IntEventerInfo->OnStatusChange != 
# 230 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                                                                          ((void *)0)
# 230 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                                                                              ))
...
static void USB_CDC_InterfaceReqHandler(pUSBSETUP Setup)
{
    ...
    case 0x20:
        if (USB_GetEPStage(USB_EP0) == EPSTAGE_IDLE)
            USB_PrepareDataReceive(USB_EP0, &CDC_LineCoding, ({ typeof(Setup->wLength) _a = Setup->wLength; typeof(sizeof(CDC_LineCoding)) _b = sizeof(CDC_LineCoding); (_a < _b) ? _a : _b; }));
        else
        {
        ...
        break;
    case 0x21:
        USB_PrepareDataTransmit(USB_EP0, &CDC_LineCoding, ({ typeof(Setup->wLength) _a = Setup->wLength; typeof(sizeof(CDC_LineCoding)) _b = sizeof(CDC_LineCoding); (_a < _b) ? _a : _b; }));
        break;
    case 0x22:



        USB_CDC_ConnectHandler((Setup->wValue & (1 << 0)) != 0);
        break;
    default:
        Error = true;
        break;
...
static boolean USB_CDC_WaitTXIdle(void)
{
    ...
    while(USB_GetEPStage(USB_EP1IN) != EPSTAGE_IDLE) {}
    ...
    {
        if (IntEventerInfo->OnStatusChange != 
# 312 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                                             ((void *)0)
# 312 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                                                 )
...
{
    if (EPAddress == ((((USB_EP1IN) < USB_EP1OUT) ? (USB_EP1IN) : (USB_EP1IN) - USB_EP1OUT + USB_EP1IN) | USB_DIR_IN))
    {
        if (USB_GetEPStage(USB_EP1IN) == EPSTAGE_IN)
        {
            uint32_t CurrentlyTransmitted = USB_GetDataAmount(USB_EP1IN) - CDC_PrevTransmitAmount;

            CDC_PrevTransmitAmount = USB_GetDataAmount(USB_EP1IN);

            if (USB_CDC_WaitTXAck)
            {
                USB_SetEPStage(USB_EP1IN, EPSTAGE_IDLE);
                USB_CDC_StopTXTimeout();
                USB_CDC_WaitTXAck = false;
            }
            else
            {
                if (IntEventerInfo->OnDataTransmitted != 
# 337 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                                                        ((void *)0)
# 337 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                                                            )
                {
                    USB_CDC_StopTXTimeout();
                    IntEventerInfo->OnDataTransmitted(CurrentlyTransmitted);
                }
                USB_CDC_WaitTXAck = USB_DataTransmit(USB_EP1IN);
                USB_CDC_RestartTXTimeout();
            }
        }
    }
    else if (EPAddress == ((((USB_EP1OUT) < USB_EP1OUT) ? (USB_EP1OUT) : (USB_EP1OUT) - USB_EP1OUT + USB_EP1IN) | USB_DIR_OUT))
    {
        uint32_t ReceivedCount;

        USB_DataReceive(USB_EP1OUT);
        if (USB_CDC_Connected)
        {
            if ((ReceivedCount = USB_GetDataAmount(USB_EP1OUT)) != 0)
                RB_WriteData(CDC_OUTRingBuffer, CDC_OUTBuffer, ReceivedCount);

            if (((ReceivedCount = RB_GetCurrentDataCount(CDC_OUTRingBuffer)) != 0) &&
                    (IntEventerInfo->OnDataReceived != 
# 358 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                                                      ((void *)0)
# 358 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                                                          ))
                IntEventerInfo->OnDataReceived(ReceivedCount);
        }
        USB_PrepareDataReceive(USB_EP1OUT, CDC_OUTBuffer, sizeof(CDC_OUTBuffer));
    }
}
...
void *USB_CDC_Initialize(void)
{
    ...
    if (USB_GetDeviceState() == USB_DEVICE_OFF)
    {
        ...
        return 
# 376 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
              ((void *)0)
# 376 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                  ;
    }

    if (CDC_TimeoutTimer == 
# 379 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                           ((void *)0)
# 379 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                               )
        CDC_TimeoutTimer = LRT_Create(500, USB_CDC_TXTimeoutHandler, TF_DIRECT);
    if (CDC_TimeoutTimer == 
# 381 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                           ((void *)0)
# 381 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                               ) return 
# 381 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                                        ((void *)0)
# 381 "Source\\Application\\Drivers\\usbdevice_cdc.c"
    ...
    USB_SetupEndpoint(USB_EP3IN, USB_CDC_CtlHandler, 16);
    USB_SetEndpointEnabled(USB_EP3IN, true);

    USB_SetupEndpoint(USB_EP1IN, USB_CDC_DataHandler, 64);
    USB_SetEndpointEnabled(USB_EP1IN, true);

    USB_SetupEndpoint(USB_EP1OUT, USB_CDC_DataHandler, 64);
    USB_PrepareDataReceive(USB_EP1OUT, CDC_OUTBuffer, sizeof(CDC_OUTBuffer));
    USB_SetEndpointEnabled(USB_EP1OUT, true);
...
{
    if ((EventerInfo != 
# 410 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                       ((void *)0)
# 410 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                           ) && (IntEventerInfo == 
# 410 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                                                   ((void *)0)
# 410 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                                                       ) && CDCInterfaceInitialized)
    {
        ...
        uint32_t RingBufferSize = ({ typeof(EventerInfo->OutBufferSize) _a = EventerInfo->OutBufferSize; typeof((2 * 64)) _b = (2 * 64); (_a > _b) ? _a : _b; });

        if (CDC_OUTRingBuffer == 
# 415 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                                ((void *)0)
# 415 "Source\\Application\\Drivers\\usbdevice_cdc.c"
        ...
        if (tmpRingBuffer != 
# 426 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                            ((void *)0)
# 426 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                                )
        {
            uint32_t intflags = __disable_interrupts();
...
{
    if ((EventerInfo != 
# 442 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                       ((void *)0)
# 442 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                           ) && (EventerInfo == IntEventerInfo))
    {
        uint32_t intflags = __disable_interrupts();
        ...
        CDC_OUTRingBuffer = RB_Destroy(CDC_OUTRingBuffer);
        IntEventerInfo = 
# 448 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                        ((void *)0)
# 448 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                            ;
        __restore_interrupts(intflags);
...
{
    if ((EventerInfo != 
# 458 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                       ((void *)0)
# 458 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                           ) && (EventerInfo == IntEventerInfo) &&
            (DataPtr != 
# 459 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                       ((void *)0)
# 459 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                           ) && CDCInterfaceInitialized)
    {
...
uint32_t USB_CDC_Write(pCDCEVENTER EventerInfo, uint8_t *DataPtr, uint32_t Count)
{
    ...
    if (USB_CDC_Connected && (DataPtr != 
# 470 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                                        ((void *)0)
# 470 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                                            ) && Count && CDCInterfaceInitialized &&
            (EventerInfo != 
# 471 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                           ((void *)0)
# 471 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                               ) && (EventerInfo == IntEventerInfo))
        do
        {
            ...
            USB_PrepareDataTransmit(USB_EP1IN, DataPtr, Count);
            USB_DataTransmit(USB_EP1IN);
...
{
    if ((EventerInfo != 
# 491 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                       ((void *)0)
# 491 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                           ) && (EventerInfo == IntEventerInfo) && CDCInterfaceInitialized)
    {
        uint32_t intflags = __disable_interrupts();
...
{
    if ((EventerInfo != 
# 504 "Source\\Application\\Drivers\\usbdevice_cdc.c" 3 4
                       ((void *)0)
# 504 "Source\\Application\\Drivers\\usbdevice_cdc.c"
                           ) && (EventerInfo == IntEventerInfo) && CDCInterfaceInitialized)
    {
        uint32_t intflags = __disable_interrupts();
