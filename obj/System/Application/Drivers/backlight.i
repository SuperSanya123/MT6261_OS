# 1 "Source\\Application\\Drivers\\backlight.c"
# 1 "C:\\Users\\user\\Desktop\\DZ09-master//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "Source\\Application\\Drivers\\backlight.c"
# 21 "Source\\Application\\Drivers\\backlight.c"
...
# 22 "Source\\Application\\Drivers\\backlight.c" 2
# 1 "Source\\Application\\Drivers\\backlight.h" 1
# 23 "Source\\Application\\Drivers\\backlight.c" 2
...
{
    Value = ({ typeof(Value) _a = Value; typeof(100) _b = 100; (_a < _b) ? _a : _b; });

    if (BLState.Mode == BM_ISINK)
    {
        if (Value < 20) PMU_SetISINKParameters(ISINK_CH0, IC_4mA, TurnOn);
        else if (Value < 40) PMU_SetISINKParameters(ISINK_CH0, IC_8mA, TurnOn);
        else if (Value < 60) PMU_SetISINKParameters(ISINK_CH0, IC_12mA, TurnOn);
        else if (Value < 80) PMU_SetISINKParameters(ISINK_CH0, IC_16mA, TurnOn);
        else PMU_SetISINKParameters(ISINK_CH0, IC_20mA, TurnOn);
    }
    else if (BLState.Mode == BM_PWM)
    {
        ...
        PMU_SetISINKParameters(ISINK_CH0, IC_20mA, TurnOn);
        if (!TurnOn)
        {
            PWM_SetPowerDown(PWM_CHANNEL3, true);
            GUI_SetLockState(true);
        }
        else
        {
            Value = 10 + (Value * (100 - 10)) / 100;
            PWM_SetDutyCycle(PWM_CHANNEL3, Value);
            PWM_SetPowerDown(PWM_CHANNEL3, false);
        }
    }
}
...
static void PMUBL_HideTimerHandler(pTIMER Timer)
{
    ...
    else
    {
        ...
        LRT_SetInterval(BLReduceTimer, 30000);
    }
}

void PMUBL_Initialize(void)
{
    BLReduceTimer = LRT_Create(10000, PMUBL_HideTimerHandler, TF_DIRECT | TF_AUTOREPEAT);
    memset(&BLState, 0x00, sizeof(TBLSTATE));

    PMU_DisableISINKs();
    PWM_SetupChannel(PWM_CHANNEL3, 163, 0, PWF_FSEL_32K | PWF_CLKDIV1);
    PMUBL_SetBacklightMode(BM_PWM);

    PMUBL_TurnOn(false);
    PMUBL_SetupValue(100);
}
...
void PMUBL_RestartBacklightTimer(boolean UnlockGUI)
{
    ...
    LRT_SetInterval(BLReduceTimer, 10000);
