# 1 "Source\\System\\ringbuf.c"
# 1 "C:\\Users\\user\\Desktop\\DZ09-master//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "Source\\System\\ringbuf.c"
# 21 "Source\\System\\ringbuf.c"
...
# 22 "Source\\System\\ringbuf.c" 2
# 1 "Source\\System\\ringbuf.h" 1
# 23 "Source\\System\\ringbuf.c" 2

pRINGBUF RB_Create(uint32_t BufferSize)
{
    pRINGBUF tmpRingBuf = 
# 26 "Source\\System\\ringbuf.c" 3 4
                         ((void *)0)
# 26 "Source\\System\\ringbuf.c"
                             ;
    ...
    if (BufferSize)
    {
        ...
        if (tmpRingBuf == 
# 32 "Source\\System\\ringbuf.c" 3 4
                         ((void *)0)
# 32 "Source\\System\\ringbuf.c"
                             ) return 
# 32 "Source\\System\\ringbuf.c" 3 4
                                      ((void *)0)
# 32 "Source\\System\\ringbuf.c"
        ...
        tmpRingBuf->Tail = 
# 35 "Source\\System\\ringbuf.c" 3 4
                          ((void *)0)
# 35 "Source\\System\\ringbuf.c"
                              ;
...
{
    if (RingBuffer != 
# 43 "Source\\System\\ringbuf.c" 3 4
                     ((void *)0)
# 43 "Source\\System\\ringbuf.c"
                         )
    {
    ...
    return 
# 50 "Source\\System\\ringbuf.c" 3 4
          ((void *)0)
# 50 "Source\\System\\ringbuf.c"
              ;
}
...
uint32_t RB_WriteByte(pRINGBUF RingBuffer, uint8_t Data)
{
    ...
    if (RingBuffer != 
# 57 "Source\\System\\ringbuf.c" 3 4
                     ((void *)0)
# 57 "Source\\System\\ringbuf.c"
                         )
    {
        uint32_t intflags = __disable_interrupts();
        ...
        *RingBuffer->Head++ = Data;
        ...
        if (RingBuffer->Tail == RingBuffer->Head - 1) RingBuffer->Tail++;
        else if (RingBuffer->Tail == 
# 65 "Source\\System\\ringbuf.c" 3 4
                                    ((void *)0)
# 65 "Source\\System\\ringbuf.c"
                                        ) RingBuffer->Tail = RingBuffer->Head - 1;
...
uint32_t RB_WriteData(pRINGBUF RingBuffer, uint8_t *Data, uint32_t Count)
{
    ...
    if ((RingBuffer != 
# 81 "Source\\System\\ringbuf.c" 3 4
                      ((void *)0)
# 81 "Source\\System\\ringbuf.c"
                          ) && (Data != 
# 81 "Source\\System\\ringbuf.c" 3 4
                                        ((void *)0)
# 81 "Source\\System\\ringbuf.c"
                                            ) && Count)
    {
        ...
        if (Count >= RingBuffer->BufferSize)
        {
            ...
            RingBuffer->Tail = 
# 91 "Source\\System\\ringbuf.c" 3 4
                              ((void *)0)
# 91 "Source\\System\\ringbuf.c"
                                  ;
        }
        if (RingBuffer->Tail == 
# 93 "Source\\System\\ringbuf.c" 3 4
                               ((void *)0)
# 93 "Source\\System\\ringbuf.c"
                                   ) RingBuffer->Tail = RingBuffer->Head;

        WCount = Count;
        while(Count)
        {
            uint32_t NWrite = ({ typeof(Count) _a = Count; typeof(Buflimit - (uintptr_t)RingBuffer->Head) _b = Buflimit - (uintptr_t)RingBuffer->Head; (_a < _b) ? _a : _b; });
...
uint32_t RB_ReadByte(pRINGBUF RingBuffer, uint8_t *Data)
{
    ...
    if ((RingBuffer != 
# 115 "Source\\System\\ringbuf.c" 3 4
                      ((void *)0)
# 115 "Source\\System\\ringbuf.c"
                          ) && (Data != 
# 115 "Source\\System\\ringbuf.c" 3 4
                                        ((void *)0)
# 115 "Source\\System\\ringbuf.c"
                                            ))
    {
        ...
        if (RingBuffer->Tail != 
# 119 "Source\\System\\ringbuf.c" 3 4
                               ((void *)0)
# 119 "Source\\System\\ringbuf.c"
                                   )
        {
            uintptr_t Buflimit = (uintptr_t)RingBuffer->Buffer + RingBuffer->BufferSize;
            ...
            *Data = *RingBuffer->Tail++;
            ...
            if ((uintptr_t)RingBuffer->Tail >= Buflimit) RingBuffer->Tail -= RingBuffer->BufferSize;
            if (RingBuffer->Tail == RingBuffer->Head) RingBuffer->Tail = 
# 126 "Source\\System\\ringbuf.c" 3 4
                                                                        ((void *)0)
# 126 "Source\\System\\ringbuf.c"
                                                                            ;
...
uint32_t RB_ReadData(pRINGBUF RingBuffer, uint8_t *Data, uint32_t Count)
{
    ...
    if ((RingBuffer != 
# 139 "Source\\System\\ringbuf.c" 3 4
                      ((void *)0)
# 139 "Source\\System\\ringbuf.c"
                          ) && (Data != 
# 139 "Source\\System\\ringbuf.c" 3 4
                                        ((void *)0)
# 139 "Source\\System\\ringbuf.c"
    ...
    {
        uint32_t intflags = __disable_interrupts();
        uint32_t RCount = ({ typeof(Count) _a = Count; typeof(RB_GetCurrentDataCount(RingBuffer)) _b = RB_GetCurrentDataCount(RingBuffer); (_a < _b) ? _a : _b; });
        uintptr_t Buflimit = (uintptr_t)RingBuffer->Buffer + RingBuffer->BufferSize;

        Count = RCount;
        while(Count)
        {
            uint32_t NRead = ({ typeof(Count) _a = Count; typeof(Buflimit - (uintptr_t)RingBuffer->Tail) _b = Buflimit - (uintptr_t)RingBuffer->Tail; (_a < _b) ? _a : _b; });
        ...
        if (RingBuffer->Tail == RingBuffer->Head) RingBuffer->Tail = 
# 156 "Source\\System\\ringbuf.c" 3 4
                                                                    ((void *)0)
# 156 "Source\\System\\ringbuf.c"
                                                                        ;
...
uint32_t RB_GetCurrentDataCount(pRINGBUF RingBuffer)
{
    ...
    if (RingBuffer != 
# 167 "Source\\System\\ringbuf.c" 3 4
                     ((void *)0)
# 167 "Source\\System\\ringbuf.c"
                         )
    {
        uint32_t intflags = __disable_interrupts();

        if (RingBuffer->Tail == 
# 171 "Source\\System\\ringbuf.c" 3 4
                               ((void *)0)
# 171 "Source\\System\\ringbuf.c"
                                   ) DSize = 0;
...
uint32_t RB_GetCurrentFreeSpace(pRINGBUF RingBuffer)
{
    ...
    if (RingBuffer != 
# 188 "Source\\System\\ringbuf.c" 3 4
                     ((void *)0)
# 188 "Source\\System\\ringbuf.c"
                         )
    {
        uint32_t intflags = __disable_interrupts();
...
{
    if (RingBuffer != 
# 202 "Source\\System\\ringbuf.c" 3 4
                     ((void *)0)
# 202 "Source\\System\\ringbuf.c"
                         )
    {
        uint32_t intflags = __disable_interrupts();
        ...
        RingBuffer->Head = RingBuffer->Buffer;
        RingBuffer->Tail = 
# 207 "Source\\System\\ringbuf.c" 3 4
                          ((void *)0)
# 207 "Source\\System\\ringbuf.c"
                              ;
        __restore_interrupts(intflags);
    }
}
