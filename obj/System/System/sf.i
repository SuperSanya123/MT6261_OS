# 1 "Source\\System\\sf.c"
# 1 "C:\\Users\\user\\Desktop\\DZ09-master//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "Source\\System\\sf.c"
# 21 "Source\\System\\sf.c"
...
# 22 "Source\\System\\sf.c" 2
# 1 "Source\\System\\sf.h" 1
# 23 "Source\\System\\sf.c" 2
...
static void __attribute__((section(".ramfunc"))) SF_DevWaitReady(TSFI_CS CS)
{
    uint8_t tmpSR;

    do
    {
        SFI_DeviceCommandRead(CS, 0X05, &tmpSR, 1);
    }
    while(tmpSR & (1 << 0));
}

static boolean __attribute__((section(".ramfunc"))) SF_BlankCheck(void *Address, size_t Count)
{
...
static void __attribute__((section(".ramfunc"))) SF_EraseBlock64K(TSFI_CS CS, void *Address)
{
    uint32_t intflags = __disable_interrupts();

    SFI_DeviceCommandWrite(CS, 0X06, 
# 63 "Source\\System\\sf.c" 3 4
                                           ((void *)0)
# 63 "Source\\System\\sf.c"
                                               , 0);
    SFI_DeviceCmdAddrWrite(CS, 0XD8, (uintptr_t)Address, 
# 64 "Source\\System\\sf.c" 3 4
                                                                        ((void *)0)
# 64 "Source\\System\\sf.c"
                                                                            , 0);
    SF_DevWaitReady(CS);
...
static void __attribute__((section(".ramfunc"))) SF_EraseBlock32K(TSFI_CS CS, void *Address)
{
    uint32_t intflags = __disable_interrupts();

    SFI_DeviceCommandWrite(CS, 0X06, 
# 74 "Source\\System\\sf.c" 3 4
                                           ((void *)0)
# 74 "Source\\System\\sf.c"
                                               , 0);
    SFI_DeviceCmdAddrWrite(CS, 0X52, (uintptr_t)Address, 
# 75 "Source\\System\\sf.c" 3 4
                                                                        ((void *)0)
# 75 "Source\\System\\sf.c"
                                                                            , 0);
    SF_DevWaitReady(CS);
...
static void __attribute__((section(".ramfunc"))) SF_EraseBlock4K(TSFI_CS CS, void *Address)
{
    uint32_t intflags = __disable_interrupts();

    SFI_DeviceCommandWrite(CS, 0X06, 
# 85 "Source\\System\\sf.c" 3 4
                                           ((void *)0)
# 85 "Source\\System\\sf.c"
                                               , 0);
    SFI_DeviceCmdAddrWrite(CS, 0X20, (uintptr_t)Address, 
# 86 "Source\\System\\sf.c" 3 4
                                                                       ((void *)0)
# 86 "Source\\System\\sf.c"
                                                                           , 0);
    SF_DevWaitReady(CS);
...
static void __attribute__((section(".ramfunc"))) SF_WriteBlock(TSFI_CS CS, void *Address, uint8_t *Data, size_t Count)
{
    uint32_t intflags = __disable_interrupts();

    SFI_DeviceCommandWrite(CS, 0X06, 
# 96 "Source\\System\\sf.c" 3 4
                                           ((void *)0)
# 96 "Source\\System\\sf.c"
                                               , 0);
    SFI_DeviceCmdAddrWrite(CS, 0X02, (uintptr_t)Address, Data, Count);
    SF_DevWaitReady(CS);
    __restore_interrupts(intflags);
}

uint32_t __attribute__((section(".ramfunc"))) SF_DevReadID(TSFI_CS CS)
{
    uint32_t tmpDevixeID = 0, intflags = __disable_interrupts();

    SFI_DeviceCommandRead(CS, 0X9F, (uint8_t *)&tmpDevixeID, 3);
    __restore_interrupts(intflags);

    return tmpDevixeID;
}

boolean __attribute__((section(".ramfunc"))) SF_WriteStatus(TSFI_CS CS, uint8_t *Data, size_t Count)
{
    boolean Result = false;

    if ((FlashConfig != 
# 116 "Source\\System\\sf.c" 3 4
                       ((void *)0)
# 116 "Source\\System\\sf.c"
                           ) && (Data != 
# 116 "Source\\System\\sf.c" 3 4
                                         ((void *)0)
# 116 "Source\\System\\sf.c"
                                             ) && Count)
    {
        ...
        Result = SFI_DeviceCommandWrite(CS, 0X06, 
# 120 "Source\\System\\sf.c" 3 4
                                                        ((void *)0)
# 120 "Source\\System\\sf.c"
                                                            , 0);
        Result &= SFI_DeviceCommandWrite(CS, 0X01, Data, Count);
        __restore_interrupts(intflags);
    }
    return Result;
}

size_t __attribute__((section(".ramfunc"))) SF_Read(TSFI_CS CS, void *Address, uint8_t *Data, size_t Count)
{
    if ((CS < SFI_CSNUM) && (Data != 
# 129 "Source\\System\\sf.c" 3 4
                                    ((void *)0)
# 129 "Source\\System\\sf.c"
                                        ) && ((uintptr_t)Address < FlashCapacity))
    {
        uint8_t *pReadData = (uint8_t *)((uintptr_t)&__ROMBase + (uintptr_t)Address);

        Count = ({ typeof(Count) _a = Count; typeof(FlashCapacity - (uintptr_t)Address) _b = FlashCapacity - (uintptr_t)Address; (_a < _b) ? _a : _b; });
...
size_t __attribute__((section(".ramfunc"))) SF_Write(TSFI_CS CS, void *Address, uint8_t *Data, size_t Count)
{
    if ((CS < SFI_CSNUM) && (Data != 
# 143 "Source\\System\\sf.c" 3 4
                                    ((void *)0)
# 143 "Source\\System\\sf.c"
                                        ) && ((uintptr_t)Address < FlashCapacity))
    {
        uint8_t *bAddress = Address;

        Count = ({ typeof(Count) _a = Count; typeof(FlashCapacity - (uintptr_t)bAddress) _b = FlashCapacity - (uintptr_t)bAddress; (_a < _b) ? _a : _b; });

        while(Count)
        {
            uint32_t nBytes = FlashConfig->PageSize - ((uintptr_t)bAddress & (FlashConfig->PageSize - 1));

            nBytes = ({ typeof(({ typeof(nBytes) _a = nBytes; typeof(Count) _b = Count; (_a < _b) ? _a : _b; })) _a = ({ typeof(nBytes) _a = nBytes; typeof(Count) _b = Count; (_a < _b) ? _a : _b; }); typeof(0x0080) _b = 0x0080; (_a < _b) ? _a : _b; });
...
boolean __attribute__((section(".ramfunc"))) SF_Erase(TSFI_CS CS, void *Address, size_t Count)
{
    boolean Result = false;
    ...
    {
        uint8_t *bAddress = Address;
        uint32_t AlignMask = (FlashConfig->EraseSupport & BR_4K) ? 0x0FFF :
                             (FlashConfig->EraseSupport & BR_32K) ? 0x7FFF :
                             (FlashConfig->EraseSupport & BR_64K) ? 0xFFFF : 0;

        if (AlignMask)
        {
            size_t nStart, nEnd, nBlocks;
            uint8_t *pStart = 
# 181 "Source\\System\\sf.c" 3 4
                             ((void *)0)
# 181 "Source\\System\\sf.c"
                                 , *pEnd = 
# 181 "Source\\System\\sf.c" 3 4
                                           ((void *)0)
# 181 "Source\\System\\sf.c"
            ...
            if (nStart && !SF_BlankCheck(bAddress, nStart) &&
                    ((pStart = malloc(nStart)) != 
# 195 "Source\\System\\sf.c" 3 4
                                                 ((void *)0)
# 195 "Source\\System\\sf.c"
                                                     ))
                SF_Read(CS, bAddress, pStart, nStart);
            if (nEnd && !SF_BlankCheck((void *)((uintptr_t)bAddress + nStart + Count), nEnd) &&
                    ((pEnd = malloc(nEnd)) != 
# 198 "Source\\System\\sf.c" 3 4
                                             ((void *)0)
# 198 "Source\\System\\sf.c"
                                                 ))
            ...
            while(nBlocks)
            {
                if ((FlashConfig->EraseSupport & BR_64K) &&
                        !((uintptr_t)bAddress & 0xFFFF) && (nBlocks >= 16))
                {

                    if (!SF_BlankCheck(bAddress, 0xFFFF + 1))
                        SF_EraseBlock64K(CS, bAddress);
                    nBlocks -= 16;
                    bAddress += 0xFFFF + 1;
                }
                else if ((FlashConfig->EraseSupport & BR_32K) &&
                         !((uintptr_t)bAddress & 0x7FFF) && (nBlocks >= 8))
                {

                    if (!SF_BlankCheck(bAddress, 0x7FFF + 1))
                ...
                else
                {
                    ...
                    if (!SF_BlankCheck(bAddress, 0x0FFF + 1))
            ...
            if (pStart != 
# 232 "Source\\System\\sf.c" 3 4
                         ((void *)0)
# 232 "Source\\System\\sf.c"
                             )
            {
            ...
            if (pEnd != 
# 237 "Source\\System\\sf.c" 3 4
                       ((void *)0)
# 237 "Source\\System\\sf.c"
                           )
            {
...
boolean SF_Initialize(void)
{
    ...
    do
    {
        ...
        {
            do { printf(" Current interface mode: UNKNOWN!\r\n"); fflush(
# 263 "Source\\System\\sf.c" 3
           (_impure_ptr->_stdout)
# 263 "Source\\System\\sf.c"
           ); } while(0);
            break;
        }
        do { printf(" Current interface mode: %s\r\n", s); fflush(
# 266 "Source\\System\\sf.c" 3
       (_impure_ptr->_stdout)
# 266 "Source\\System\\sf.c"
       ); } while(0);

        if (DeviceID && (DeviceID != 0xFFFFFFFF))
            do { printf(" Found device with ID: (0x%02X, 0x%02X, 0x%02X)\r\n", DeviceID & 0xFF, (DeviceID >> 8) & 0xFF, (DeviceID >> 16) & 0xFF); fflush(
# 269 "Source\\System\\sf.c" 3
           (_impure_ptr->_stdout)
# 269 "Source\\System\\sf.c"
           ); } while(0)
                                                                                        ;
        else
        {
            do { printf(" Something went wrong, can not read device ID!\r\n"); fflush(
# 273 "Source\\System\\sf.c" 3
           (_impure_ptr->_stdout)
# 273 "Source\\System\\sf.c"
           ); } while(0);
            break;
        }

        FlashConfig = 
# 277 "Source\\System\\sf.c" 3 4
                     ((void *)0)
# 277 "Source\\System\\sf.c"
                         ;
        FlashCapacity = 0;
        SFI_DeviceCmdAddrWrite = 
# 279 "Source\\System\\sf.c" 3 4
                                ((void *)0)
# 279 "Source\\System\\sf.c"
                                    ;
        while(pConfig->DeviceID != 0)
            if (pConfig->DeviceID == DeviceID)
            {
                FlashConfig = pConfig;
                do { printf(" Found supported device \"%s\"\r\n", FlashConfig->DeviceName); fflush(
# 284 "Source\\System\\sf.c" 3
               (_impure_ptr->_stdout)
# 284 "Source\\System\\sf.c"
               ); } while(0);
                break;
            }
            else pConfig++;
        ...
        if (FlashConfig != 
# 289 "Source\\System\\sf.c" 3 4
                          ((void *)0)
# 289 "Source\\System\\sf.c"
                              )
        {
            ...
            do { printf(" Total capacity - %u KiB\r\n", FlashCapacity >> 10); fflush(
# 296 "Source\\System\\sf.c" 3
           (_impure_ptr->_stdout)
# 296 "Source\\System\\sf.c"
           ); } while(0);

            do { printf(" Applying SF device configuration..."); fflush(
# 298 "Source\\System\\sf.c" 3
           (_impure_ptr->_stdout)
# 298 "Source\\System\\sf.c"
           ); } while(0);
            SFI_ConfigureInterface(SFI_CS0, pConfig);
            do { printf("Complete.\r\n"); fflush(
# 300 "Source\\System\\sf.c" 3
           (_impure_ptr->_stdout)
# 300 "Source\\System\\sf.c"
           ); } while(0);
        }
        else
        {
            do { printf(" No configuration found for device (0x%02X, 0x%02X, 0x%02X)\r\n", DeviceID & 0xFF, (DeviceID >> 8) & 0xFF, (DeviceID >> 16) & 0xFF); fflush(
# 304 "Source\\System\\sf.c" 3
           (_impure_ptr->_stdout)
# 304 "Source\\System\\sf.c"
           ); } while(0)
                                                                                        ;
            break;
        }
    ...
    if (!Result) do { printf(" SFI initialization failed.\r\n"); fflush(
# 312 "Source\\System\\sf.c" 3
                (_impure_ptr->_stdout)
# 312 "Source\\System\\sf.c"
                ); } while(0);
