# 1 "Source\\Lib\\MT6261\\Drivers\\usb.c"
# 1 "C:\\Users\\user\\Desktop\\DZ09-master//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "Source\\Lib\\MT6261\\Drivers\\usb.c"
# 21 "Source\\Lib\\MT6261\\Drivers\\usb.c"
...
# 22 "Source\\Lib\\MT6261\\Drivers\\usb.c" 2
# 1 ".\\Source\\Lib\\USB/usb9.h" 1
# 112 ".\\Source\\Lib\\USB/usb9.h"
...
# 119 ".\\Source\\Lib\\USB/usb9.h" 3
 __attribute__((__packed__)) 
# 119 ".\\Source\\Lib\\USB/usb9.h"
          TUSBSETUP, *pUSBSETUP;
...
# 23 "Source\\Lib\\MT6261\\Drivers\\usb.c" 2
# 1 "Source\\Lib\\MT6261\\Drivers\\usb.h" 1
# 24 "Source\\Lib\\MT6261\\Drivers\\usb.c" 2
...
static void USB_EPFIFORead(TEP Endpoint, uint32_t Count, void *Data)
{
    ...
    if (Endpoint >= USB_EP1OUT) Endpoint = Endpoint - USB_EP1OUT + USB_EP1IN;
    (*(volatile uint8_t *)(0xA0900000 + 0x000E)) = Endpoint;

    while(Count--)
        if (DestPointer != 
# 49 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3 4
                          ((void *)0)
# 49 "Source\\Lib\\MT6261\\Drivers\\usb.c"
                              ) *DestPointer++ = (*(volatile uint8_t *)(0xA0900000 + 0x0020 + 4 * (Endpoint)));
        else if ((*(volatile uint8_t *)(0xA0900000 + 0x0020 + 4 * (Endpoint)))) {};
}
...
static void USB_EPFIFOWrite(TEP Endpoint, uint32_t Count, void *Data)
{
    ...
    while(Count--)
        (*(volatile uint8_t *)(0xA0900000 + 0x0020 + 4 * (Endpoint))) = *SrcPointer++;
}
...
static void USB_EP0Handler(uint8_t EPAddress)
{
    if (EPState[USB_EP0].Stage == EPSTAGE_IDLE)
    {
        ...
        else if (Count) USB_UpdateEPState(USB_EP0, USB_DIR_OUT, true, true);
    }
...
{
    (*(volatile uint8_t *)(0xA0900000 + 0x0001)) = (1 << 4);

    (*(volatile uint8_t *)(0xA0900000 + 0x000F)) = (1 << 7);
    (*(volatile uint8_t *)(0xA0900000 + 0x000F)) = 0;

    (*(volatile uint8_t *)(0xA0900000 + 0x0000)) = (1 << 7);
    (*(volatile uint8_t *)(0xA0900000 + 0x0007)) = 0;
    (*(volatile uint8_t *)(0xA0900000 + 0x0009)) = 0;
    (*(volatile uint8_t *)(0xA0900000 + 0x000B)) = (1 << 0) | (1 << 1) | (1 << 2);
    ...
    {
        do { printf("Can not initialize USB device interface.\r\n"); fflush(
# 109 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3
       (_impure_ptr->_stdout)
# 109 "Source\\Lib\\MT6261\\Drivers\\usb.c"
       ); } while(0);
        USB_DisableDevice();
    }
    else
    {
        USB_SetupEndpoint(USB_EP0, USB_EP0Handler, 64);
...
{
    *IntUSB = (*(volatile uint8_t *)(0xA0900000 + 0x0006)) & (*(volatile uint8_t *)(0xA0900000 + 0x000B));
    *IntIN = (*(volatile uint8_t *)(0xA0900000 + 0x0002)) & (*(volatile uint8_t *)(0xA0900000 + 0x0007));
    *IntOUT = (*(volatile uint8_t *)(0xA0900000 + 0x0004)) & (*(volatile uint8_t *)(0xA0900000 + 0x0009));
...
static void USB_EPDefaultHandler(uint8_t EPAddress)
{
    ...
    (*(volatile uint8_t *)(0xA0900000 + 0x000E)) = EPIndex;
    if (!EPIndex)
    {
        tmpCSR = (*(volatile uint8_t *)(0xA0900000 + 0x0011));
        if (tmpCSR & (1 << 2)) (*(volatile uint8_t *)(0xA0900000 + 0x0011)) = tmpCSR & ~(1 << 2);
        if (tmpCSR & (1 << 4)) (*(volatile uint8_t *)(0xA0900000 + 0x0011)) = (1 << 7);
    }
    else if ((EPAddress & USB_DIR_MASK) == USB_DIR_IN)
    {
        tmpCSR = (*(volatile uint8_t *)(0xA0900000 + 0x0011));
        (*(volatile uint8_t *)(0xA0900000 + 0x0011)) = tmpCSR & ~(1 << 5);
    }
    else
    {
        tmpCSR = (*(volatile uint8_t *)(0xA0900000 + 0x0014));
        (*(volatile uint8_t *)(0xA0900000 + 0x0014)) = tmpCSR & ~(1 << 6);
    }
}
...
static void USB_InterruptHandler(void)
{
    ...
    while(USB_GetInterruptFlags(&IntFlagsUSB, &IntFlagsIN, &IntFlagsOUT))
    {
        ...
        if (IntFlagsUSB & (1 << 1))
        {
            ...
            (*(volatile uint8_t *)(0xA0900000 + 0x000B)) = (1 << 0) | (1 << 2);
            (*(volatile uint8_t *)(0xA0900000 + 0x0001)) = (1 << 2) | (1 << 4);
        }

        for(i = 0; (i < USB_EPNUM) && (IntFlagsIN || IntFlagsOUT); i++)
        {
            uint8_t EPIndex = (*(volatile uint8_t *)(0xA0900000 + 0x000E)) = EPIndex = (((i) < USB_EP1OUT) ? (i) : (i) - USB_EP1OUT + USB_EP1IN);

            if ((i < USB_EP1OUT) && (IntFlagsIN & (1 << EPIndex)))
            {
                IntFlagsIN &= ~(1 << EPIndex);
                USB_EPDefaultHandler(EPIndex | USB_DIR_IN);
                if (EPState[i].EventHandler != 
# 180 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3 4
                                              ((void *)0)
# 180 "Source\\Lib\\MT6261\\Drivers\\usb.c"
            ...
            if ((i >= USB_EP1OUT) && (IntFlagsOUT & (1 << EPIndex)))
            {
                ...
                if (EPState[i].EventHandler != 
# 186 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3 4
                                              ((void *)0)
# 186 "Source\\Lib\\MT6261\\Drivers\\usb.c"
        ...
        if (IntFlagsUSB & (1 << 0))
        {
            ...
            (*(volatile uint8_t *)(0xA0900000 + 0x000B)) = (1 << 1) | (1 << 2);
            (*(volatile uint8_t *)(0xA0900000 + 0x0001)) = (1 << 0) | (1 << 4);
        }
    }
}
...
void USB_Initialize(void)
{
    ...
    do { printf("USB driver initialization.\r\n"); fflush(
# 202 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3
   (_impure_ptr->_stdout)
# 202 "Source\\Lib\\MT6261\\Drivers\\usb.c"
   ); } while(0);
    memset(&EPState, 0x00, sizeof(EPState));

    USB_DisableDevice();

    NVIC_RegisterIRQ(0x19, USB_InterruptHandler, 1, true, true);
...
void USB_EnableDevice(void)
{
    ...
    if (USBDeviceState == USB_DEVICE_OFF)
    {
        ...
        do { typeof((0x12)) _x = (0x12); if (_x <= (0x2E)) { (*(volatile uint16_t *)(0xA0010000 + 0x0320 + 0x04 * (_x >> 4))) = (1UL << (_x & 0x0F)); } else if (_x <= (0x34)) { (*(volatile uint16_t *)(0xA0730000 + 0x0010)) = (1UL << (_x & 0x0F)); } } while(0);



        USC_Pause_us(50);

        (*(volatile uint32_t *)(0xA0910000 + 0x08C4)) &= ~(1 << 15);
        (*(volatile uint32_t *)(0xA0910000 + 0x08C0)) |= (1 << 11);
        USC_Pause_us(10);

        (*(volatile uint8_t *)(0xA0900000 + 0x0240)) = (1 << 0);
        USB_SetDeviceState(USB_DEVICE_IDLE);

        NVIC_EnableIRQ(0x19);
        do { printf("USB device enabled.\r\n"); fflush(
# 235 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3
       (_impure_ptr->_stdout)
# 235 "Source\\Lib\\MT6261\\Drivers\\usb.c"
       ); } while(0);
    }
...
void USB_DisableDevice(void)
{
    if (USBDeviceState != USB_DEVICE_OFF)
    {
        ...
        do { typeof((0x12)) _x = (0x12); if (_x <= (0x2E)) { (*(volatile uint16_t *)(0xA0010000 + 0x0310 + 0x04 * (_x >> 4))) = (1UL << (_x & 0x0F)); } else if (_x <= (0x34)) { (*(volatile uint16_t *)(0xA0730000 + 0x000C)) = (1UL << (_x & 0x0F)); } } while(0);
        do { printf("USB device disabled.\r\n"); fflush(
# 262 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3
       (_impure_ptr->_stdout)
# 262 "Source\\Lib\\MT6261\\Drivers\\usb.c"
       ); } while(0);
    }
}
...
{
    (*(volatile uint8_t *)(0xA0900000 + 0x0000)) = Address;
...
void USB_SetDeviceState(TUSBSTATE State)
{
    if (USBDeviceState != State)
    {
        ...
        if ((USB_OnDeviceActive != 
# 288 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3 4
                                  ((void *)0)
# 288 "Source\\Lib\\MT6261\\Drivers\\usb.c"
...
boolean USB_SetupEndpoint(TEP Endpoint, void (*Handler)(uint8_t), uint8_t MaxPacketSize)
{
    ...
    if ((Endpoint >= USB_EPNUM) || (Handler == 
# 314 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3 4
                                              ((void *)0)
# 314 "Source\\Lib\\MT6261\\Drivers\\usb.c"
                                                  )) return false;

    MaxPacketSize = ({ typeof(MaxPacketSize) _a = MaxPacketSize; typeof(EPFIFOSize[Endpoint]) _b = EPFIFOSize[Endpoint]; (_a < _b) ? _a : _b; });

    (*(volatile uint8_t *)(0xA0900000 + 0x000E)) = (((Endpoint) < USB_EP1OUT) ? (Endpoint) : (Endpoint) - USB_EP1OUT + USB_EP1IN);
    if (Endpoint > USB_EP0)
    {
        if (Endpoint < USB_EP1OUT)
        {
            (*(volatile uint8_t *)(0xA0900000 + 0x0011)) = (1 << 6);
            if ((*(volatile uint8_t *)(0xA0900000 + 0x0011)) & (1 << 0)) (*(volatile uint8_t *)(0xA0900000 + 0x0011)) = (1 << 7) | (1 << 3);
            if ((*(volatile uint8_t *)(0xA0900000 + 0x0011)) & (1 << 0)) (*(volatile uint8_t *)(0xA0900000 + 0x0011)) = (1 << 7) | (1 << 3);
            (*(volatile uint8_t *)(0xA0900000 + 0x0011)) &= ~(1 << 7);
            (*(volatile uint8_t *)(0xA0900000 + 0x0010)) = MaxPacketSize;
        }
        else if ((Endpoint == USB_EP1OUT) || (Endpoint == USB_EP2OUT))
        {
            (*(volatile uint8_t *)(0xA0900000 + 0x0014)) = (1 << 7);
            if ((*(volatile uint8_t *)(0xA0900000 + 0x0014)) & (1 << 0)) (*(volatile uint8_t *)(0xA0900000 + 0x0014)) = (1 << 4);
            if ((*(volatile uint8_t *)(0xA0900000 + 0x0014)) & (1 << 0)) (*(volatile uint8_t *)(0xA0900000 + 0x0014)) = (1 << 4);
            (*(volatile uint8_t *)(0xA0900000 + 0x0013)) = MaxPacketSize;
...
boolean USB_SetEndpointEnabled(TEP Endpoint, boolean Enable)
{
    ...
    if ((Endpoint >= USB_EPNUM) || (EPState[Endpoint].EventHandler == 
# 353 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3 4
                                                                     ((void *)0)
# 353 "Source\\Lib\\MT6261\\Drivers\\usb.c"
                                                                         )) return false;

    (*(volatile uint8_t *)(0xA0900000 + 0x000E)) = EPIndex = (((Endpoint) < USB_EP1OUT) ? (Endpoint) : (Endpoint) - USB_EP1OUT + USB_EP1IN);
    if ((EPState[Endpoint].EPType & USB_DIR_MASK) == USB_DIR_IN)
    {
        if (Enable)
        {
            ...
            if ((*(volatile uint8_t *)(0xA0900000 + 0x0011)) & (1 << 0)) (*(volatile uint8_t *)(0xA0900000 + 0x0011)) = (1 << 7) | (1 << 3);
            (*(volatile uint8_t *)(0xA0900000 + 0x0011)) &= ~(1 << 7);
            (*(volatile uint8_t *)(0xA0900000 + 0x0012)) = 0;
            (*(volatile uint8_t *)(0xA0900000 + 0x0007)) |= 1 << EPIndex;
        }
        else
        {
            (*(volatile uint8_t *)(0xA0900000 + 0x0007)) &= ~(1 << EPIndex);
    ...
    else if ((Endpoint == USB_EP1OUT) || (Endpoint == USB_EP2OUT))
    {
        if (Enable)
        {
            ...
            if ((*(volatile uint8_t *)(0xA0900000 + 0x0014)) & (1 << 0)) (*(volatile uint8_t *)(0xA0900000 + 0x0014)) = (1 << 4);
            if ((*(volatile uint8_t *)(0xA0900000 + 0x0014)) & (1 << 0)) (*(volatile uint8_t *)(0xA0900000 + 0x0014)) = (1 << 4);
            (*(volatile uint8_t *)(0xA0900000 + 0x0015)) = 0;
            (*(volatile uint8_t *)(0xA0900000 + 0x0009)) |= 1 << EPIndex;
        }
        else
        {
            (*(volatile uint8_t *)(0xA0900000 + 0x0009)) &= ~(1 << EPIndex);
...
uint32_t USB_GetOUTDataLength(TEP Endpoint)
{
    ...
    if (Endpoint == USB_EP0)
    {
        (*(volatile uint8_t *)(0xA0900000 + 0x000E)) = Endpoint;
        tmpCSR = (*(volatile uint8_t *)(0xA0900000 + 0x0011));
        if (tmpCSR & (1 << 0)) DataLength = (*(volatile uint8_t *)(0xA0900000 + 0x0016));
    }
    else if ((Endpoint == USB_EP1OUT) || (Endpoint == USB_EP2OUT))
    {
        ...
        (*(volatile uint8_t *)(0xA0900000 + 0x000E)) = EPIndex;
        tmpCSR = (*(volatile uint8_t *)(0xA0900000 + 0x0014));
        if (tmpCSR & (1 << 0)) DataLength = ((((*(volatile uint8_t *)(0xA0900000 + 0x0017))) & 0x07) << 8) | (*(volatile uint8_t *)(0xA0900000 + 0x0016));
    }
    return DataLength;
}
...
void USB_UpdateEPState(TEP Endpoint, TUSBDIR Transaction, boolean SendStall, boolean DataEnd)
{
    ...
    if (Endpoint == USB_EP0)
    {
        tmpCSR = ((Transaction & USB_DIR_MASK) == USB_DIR_OUT) ? (1 << 6) : (1 << 1);

        (*(volatile uint8_t *)(0xA0900000 + 0x000E)) = Endpoint;
        if (SendStall) tmpCSR |= (1 << 5);
        if (DataEnd) tmpCSR |= (1 << 3);

        (*(volatile uint8_t *)(0xA0900000 + 0x0011)) = tmpCSR;
    }
    else if (Endpoint < USB_EPNUM)
    {
        ...
        {
            tmpCSR = (*(volatile uint8_t *)(0xA0900000 + 0x0011));
            if ((Transaction & USB_DIR_MASK) == USB_DIR_IN) tmpCSR |= (1 << 0);
            if (SendStall) tmpCSR |= (1 << 4);
        ...
        {
            tmpCSR = (*(volatile uint8_t *)(0xA0900000 + 0x0014));
            if ((Transaction & USB_DIR_MASK) == USB_DIR_OUT) tmpCSR &= ~(1 << 0);
            if (SendStall) tmpCSR |= (1 << 4);

            (*(volatile uint8_t *)(0xA0900000 + 0x0014)) = tmpCSR;
        }
    }
}
...
void USB_ControlEPStall(TEP Endpoint, boolean Enable)
{
    ...
    if (Enable)
    {
        if ((EPState[Endpoint].EPType & USB_DIR_MASK) == USB_DIR_IN)
        {
            ...
            tmpCSR |= (1 << 4);
            (*(volatile uint8_t *)(0xA0900000 + 0x0011)) = tmpCSR;
            if ((*(volatile uint8_t *)(0xA0900000 + 0x0011)) & (1 << 0)) (*(volatile uint8_t *)(0xA0900000 + 0x0011)) = tmpCSR | (1 << 7) | (1 << 3);
            if ((*(volatile uint8_t *)(0xA0900000 + 0x0011)) & (1 << 0)) (*(volatile uint8_t *)(0xA0900000 + 0x0011)) = tmpCSR | (1 << 7) | (1 << 3);
        ...
        else if ((Endpoint == USB_EP1OUT) || (Endpoint == USB_EP2OUT))
        {
            ...
            tmpCSR |= (1 << 5);
            (*(volatile uint8_t *)(0xA0900000 + 0x0014)) = tmpCSR;
            if ((*(volatile uint8_t *)(0xA0900000 + 0x0014)) & (1 << 0)) (*(volatile uint8_t *)(0xA0900000 + 0x0014)) = tmpCSR | (1 << 4);
            if ((*(volatile uint8_t *)(0xA0900000 + 0x0014)) & (1 << 0)) (*(volatile uint8_t *)(0xA0900000 + 0x0014)) = tmpCSR | (1 << 4);
            EPState[Endpoint].Stalled = false;
        }
    }
...
void USB_PrepareDataReceive(TEP Endpoint, void *DataBuffer, uint32_t MaxDataLength)
{
            ...
            ((Endpoint == USB_EP0) || (EPState[Endpoint].EPType & USB_DIR_MASK) == USB_DIR_OUT))
    {
        ...
        EPState[Endpoint].Stage = ((DataBuffer != 
# 511 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3 4
                                                 ((void *)0)
# 511 "Source\\Lib\\MT6261\\Drivers\\usb.c"
                                                     ) && MaxDataLength) ? EPSTAGE_OUT : EPSTAGE_IDLE;
    }
}
...
boolean USB_DataReceive(TEP Endpoint)
{
    ...
    if ((Endpoint < USB_EPNUM) &&
            ((Endpoint == USB_EP0) || (EPState[Endpoint].EPType & USB_DIR_MASK) == USB_DIR_OUT) &&
            (EPState[Endpoint].DataBuffer != 
# 521 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3 4
                                            ((void *)0)
# 521 "Source\\Lib\\MT6261\\Drivers\\usb.c"
                                                ) && (EPState[Endpoint].DataPosition != 
# 521 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3 4
                                                                                        ((void *)0)
# 521 "Source\\Lib\\MT6261\\Drivers\\usb.c"
                                                                                            ))
    {
        ...
        {
            uint32_t ReadCount = ({ typeof(Count) _a = Count; typeof(EPInfo->DataLength - TotalReceived) _b = EPInfo->DataLength - TotalReceived; (_a < _b) ? _a : _b; });

            USB_EPFIFORead(Endpoint, ReadCount, EPInfo->DataPosition);
            EPInfo->DataPosition += ReadCount;
            Count -= ReadCount;
        }
        if (Count) USB_EPFIFORead(Endpoint, Count, 
# 537 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3 4
                                                  ((void *)0)
# 537 "Source\\Lib\\MT6261\\Drivers\\usb.c"
                                                      );
...
void USB_PrepareDataTransmit(TEP Endpoint, void *DataBuffer, uint32_t DataLength)
{
            ...
            ((Endpoint == USB_EP0) || (EPState[Endpoint].EPType & USB_DIR_MASK) == USB_DIR_IN))
    {
        ...
        EPState[Endpoint].Stage = ((DataBuffer != 
# 552 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3 4
                                                 ((void *)0)
# 552 "Source\\Lib\\MT6261\\Drivers\\usb.c"
                                                     ) && DataLength) ? EPSTAGE_IN : EPSTAGE_IDLE;
    }
}
...
boolean USB_DataTransmit(TEP Endpoint)
{
    ...
    {
        pEPSTATE EPInfo = &EPState[Endpoint];
        uint32_t Count = ({ typeof(EPInfo->DataLength) _a = EPInfo->DataLength; typeof(EPInfo->PacketSize) _b = EPInfo->PacketSize; (_a < _b) ? _a : _b; });

        LastPacket = (Count < EPInfo->PacketSize);
        if (Count && (EPInfo->DataPosition != 
# 567 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3 4
                                             ((void *)0)
# 567 "Source\\Lib\\MT6261\\Drivers\\usb.c"
                                                 ))
        {
...
uint32_t USB_GetDataAmount(TEP Endpoint)
{
    if ((Endpoint < USB_EPNUM) &&
            (EPState[Endpoint].DataBuffer != 
# 582 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3 4
                                            ((void *)0)
# 582 "Source\\Lib\\MT6261\\Drivers\\usb.c"
                                                ) && (EPState[Endpoint].DataPosition != 
# 582 "Source\\Lib\\MT6261\\Drivers\\usb.c" 3 4
                                                                                        ((void *)0)
# 582 "Source\\Lib\\MT6261\\Drivers\\usb.c"
