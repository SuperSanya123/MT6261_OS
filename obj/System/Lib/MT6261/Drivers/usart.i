# 1 "Source\\Lib\\MT6261\\Drivers\\usart.c"
# 1 "C:\\Users\\user\\Desktop\\DZ09-master//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "Source\\Lib\\MT6261\\Drivers\\usart.c"
# 21 "Source\\Lib\\MT6261\\Drivers\\usart.c"
...
# 22 "Source\\Lib\\MT6261\\Drivers\\usart.c" 2
# 1 "Source\\Lib\\MT6261\\Drivers\\usart.h" 1
# 23 "Source\\Lib\\MT6261\\Drivers\\usart.c" 2

TUSART_CONTEXT USARTINFO[USART_MODULES] =
{
    {((volatile TUSARTREGS*) 0xA0080000), (0x1D)},
    {((volatile TUSARTREGS*) 0xA0090000), (0x1E)},
    {((volatile TUSARTREGS*) 0xA00A0000), (0x1F)}
};
...
# 39 "Source\\Lib\\MT6261\\Drivers\\usart.c" 3 4
                                      ((void *)0)
# 39 "Source\\Lib\\MT6261\\Drivers\\usart.c"
                                          ;
...
boolean USART_Initialize(TUSART Index, uint32_t Speed, void (*Handler)(void), uint32_t Flags)
{
    ...
    if (Index == USART_MODULE1) IRQ_CODE = 0x14;
    else if (Index == USART_MODULE2) IRQ_CODE = 0x16;
    else if (Index == USART_MODULE3) IRQ_CODE = 0x1D;
    else return false;


    NVIC_UnregisterIRQ(IRQ_CODE);

    do { typeof(USARTINFO[Index].PD_Code) _x = USARTINFO[Index].PD_Code; if (_x <= (0x2E)) { (*(volatile uint16_t *)(0xA0010000 + 0x0320 + 0x04 * (_x >> 4))) = (1UL << (_x & 0x0F)); } else if (_x <= (0x34)) { (*(volatile uint16_t *)(0xA0730000 + 0x0010)) = (1UL << (_x & 0x0F)); } } while(0);
    ...
    USART->MCR = 0x00;
    USART->FCR = (1 << 2) | (1 << 1);
    USART->LSR = USART->LSR & ~(1 << 0);
    USART->HIGHSPEED = (((0) & 0x03) << 0);
    USART->RATEFIX_AD = (0 << 2) | (0 << 1) | (0 << 0);

    if (Speed)
    {
        uint32_t BR = (52000000 + (16 * Speed) / 2) / (16 * Speed);
    ...
    {
        do { typeof(USARTINFO[Index].PD_Code) _x = USARTINFO[Index].PD_Code; if (_x <= (0x2E)) { (*(volatile uint16_t *)(0xA0010000 + 0x0310 + 0x04 * (_x >> 4))) = (1UL << (_x & 0x0F)); } else if (_x <= (0x34)) { (*(volatile uint16_t *)(0xA0730000 + 0x000C)) = (1UL << (_x & 0x0F)); } } while(0);
        return false;
    }
    ...
    USART_EnableInterrupts(Index, Flags);

    if (Flags & (USART_TXInt_EnFlag | USART_RXInt_EnFlag | USART_ERRInt_EnFlag))
    {
        if (Handler != 
# 96 "Source\\Lib\\MT6261\\Drivers\\usart.c" 3 4
                       ((void *)0)
# 96 "Source\\Lib\\MT6261\\Drivers\\usart.c"
                           )
        {
            NVIC_RegisterIRQ(IRQ_CODE, Handler, 0, true, true);
        }
    }
...
void USART_EnableInterrupts(TUSART Index, TUSART_FLAGS Flags)
{
    if (Index < USART_MODULES)
    {
        ...
        USARTINFO[Index].USART->LCR &= ~(1 << 7);

        if (Flags & USART_TXInt_EnFlag) USARTINFO[Index].USART->IER |= (1 << 1);
        if (Flags & USART_RXInt_EnFlag) USARTINFO[Index].USART->IER |= (1 << 0);
        if (Flags & USART_ERRInt_EnFlag) USARTINFO[Index].USART->IER |= (1 << 2);
...
{
    if ((Index < USART_MODULES) && (USARTINFO[Index].USART->FCR_RD & (1 << 0)))
        USARTINFO[Index].USART->FCR = (1 << 1) | (1 << 0);
}

void USART_FlashTX(TUSART Index)
{
    if ((Index < USART_MODULES) && (USARTINFO[Index].USART->FCR_RD & (1 << 0)))
        USARTINFO[Index].USART->FCR = (1 << 2) | (1 << 0);
}
...
{
    if (Str != 
# 175 "Source\\Lib\\MT6261\\Drivers\\usart.c" 3 4
              ((void *)0)
# 175 "Source\\Lib\\MT6261\\Drivers\\usart.c"
                  )
...
void USART_SendBufferPoll(TUSART Index)
{
    ...
    if (USARTInf->USART_TX_Buffer != 
# 186 "Source\\Lib\\MT6261\\Drivers\\usart.c" 3 4
                                    ((void *)0)
# 186 "Source\\Lib\\MT6261\\Drivers\\usart.c"
                                        )
    {
        volatile TUSARTREGS *tmpUSART = USARTInf->USART;
        uint8_t *Data;

        USART_DisableInterrupts(Index, USART_TXInt_EnFlag);
...
void USART_SendBufferInt(TUSART Index)
{
    ...
    if (USARTInf->USART_TX_Buffer != 
# 213 "Source\\Lib\\MT6261\\Drivers\\usart.c" 3 4
                                    ((void *)0)
# 213 "Source\\Lib\\MT6261\\Drivers\\usart.c"
                                        )
    {
        uint32_t Sent = 0;
        uint8_t *Data;
        ...
        USART_EnableInterrupts(Index, USART_TXInt_EnFlag);
    }
}
