# 1 "Source\\Lib\\MT6261\\Drivers\\pctl.c"
# 1 "C:\\Users\\user\\Desktop\\DZ09-master//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "Source\\Lib\\MT6261\\Drivers\\pctl.c"
# 21 "Source\\Lib\\MT6261\\Drivers\\pctl.c"
...
# 22 "Source\\Lib\\MT6261\\Drivers\\pctl.c" 2
# 1 "Source\\Lib\\MT6261\\Drivers\\pctl.h" 1
# 23 "Source\\Lib\\MT6261\\Drivers\\pctl.c" 2

void PCTL_Initialize(void)
{
    (*(volatile uint16_t *)(0xA0010000 + 0x0310)) = 0xFFFF;
    (*(volatile uint16_t *)(0xA0010000 + 0x0314)) = 0xFFFF;
    (*(volatile uint16_t *)(0xA0010000 + 0x0318)) = 0xF7FF;


    {
        int32_t PDCode = USART_GetPDCode(USART_MODULE1);

        if (PDCode != -1) do { typeof(PDCode) _x = PDCode; if (_x <= (0x2E)) { (*(volatile uint16_t *)(0xA0010000 + 0x0320 + 0x04 * (_x >> 4))) = (1UL << (_x & 0x0F)); } else if (_x <= (0x34)) { (*(volatile uint16_t *)(0xA0730000 + 0x0010)) = (1UL << (_x & 0x0F)); } } while(0);
    }



    (*(volatile uint16_t *)(0xA0730000 + 0x000C)) = 0xFFFF;
}

boolean PCTL_GetPeripheralPowerStatus(uint32_t Periph)
{
    if (Periph <= (0x2E)) return !((*(volatile uint16_t *)(0xA0010000 + 0x0300 + 0x04 * (Periph >> 4))) & (1 << (Periph & 0x0F)));
    else if (Periph <= (0x34)) return !((*(volatile uint16_t *)(0xA0730000 + 0x0008)) & (1 << (Periph & 0x0F)));
