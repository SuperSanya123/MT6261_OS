# 1 "Source\\Lib\\MT6261\\Drivers\\pwm.c"
# 1 "C:\\Users\\user\\Desktop\\DZ09-master//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "Source\\Lib\\MT6261\\Drivers\\pwm.c"
# 21 "Source\\Lib\\MT6261\\Drivers\\pwm.c"
...
# 22 "Source\\Lib\\MT6261\\Drivers\\pwm.c" 2
# 1 "Source\\Lib\\MT6261\\Drivers\\pwm.h" 1
# 23 "Source\\Lib\\MT6261\\Drivers\\pwm.c" 2

static TPWM_CONTEXT PWMINFO[PWM_CHANNELS] =
{
    {(TPWMREGS *)((volatile TPWMREGS *) 0xA00E0000), (0x20)},
    {(TPWMREGS *)((volatile TPWMREGS *) (0xA0740000 + 0x000C)), (0x31)},
    {(TPWMREGS *)((volatile TPWMREGS *) (0xA0740000 + 0x0018)), (0x30)},
    {(TPWMREGS *)((volatile TPWMREGS *) 0xA0280000), (0x24)},
};
...
boolean PWM_SetupChannel(TPWM Index, uint16_t Count, uint16_t Threshold, TPWM_FLAGS Flags)
{
    ...
    else
    {
        ...
        PWM->Ctrl = (Flags & PWF_FSEL_32K) ? (1 << 2) : (0 << 2);
        PWMINFO[Index].Threshold = (((Threshold) & 0x1FFF) << 0);

        if (Index == PWM_CHANNEL2)
        {

            PWMINFO[Index].Threshold &= 0x03;
            if (PWMINFO[Index].Threshold > (2 << 0)) PWMINFO[Index].Threshold = (2 << 0);
        }
        else
        {

            PWM->Ctrl |= (((Flags & PWF_CLKDIV8) & 0x03) << 0);
            PWM->Count = (((Count) & 0x1FFF) << 0);
            if (((uintptr_t)PWM == (0xA0740000 + 0x0018)) && (Flags & PWF_ALW_HIGH))
                PWM->Ctrl |= (1 << 3);
        }
        PWM->Threshold = (((Threshold) & 0x1FFF) << 0);
...
boolean PWM_SetPowerDown(TPWM Index, boolean PowerDown)
{
    ...
    else
    {
        ...
        if (AlreadyInState ^ PowerDown)
        {
            ...
            if (PowerDown)
            {
                do { typeof(PWMINFO[Index].PD_Code) _x = PWMINFO[Index].PD_Code; if (_x <= (0x2E)) { (*(volatile uint16_t *)(0xA0010000 + 0x0310 + 0x04 * (_x >> 4))) = (1UL << (_x & 0x0F)); } else if (_x <= (0x34)) { (*(volatile uint16_t *)(0xA0730000 + 0x000C)) = (1UL << (_x & 0x0F)); } } while(0);
                PWMINFO[Index].Flags &= ~PWF_ENABLED;
            }
            else
            {
                do { typeof(PWMINFO[Index].PD_Code) _x = PWMINFO[Index].PD_Code; if (_x <= (0x2E)) { (*(volatile uint16_t *)(0xA0010000 + 0x0320 + 0x04 * (_x >> 4))) = (1UL << (_x & 0x0F)); } else if (_x <= (0x34)) { (*(volatile uint16_t *)(0xA0730000 + 0x0010)) = (1UL << (_x & 0x0F)); } } while(0);
                PWMINFO[Index].Flags |= PWF_ENABLED;
            }
        }
    }
...
boolean PWM_SetCount(TPWM Index, uint16_t Count)
{
    ...
    {
        PWMINFO[Index].PWM->Count = (((Count) & 0x1FFF) << 0);
    }
    return true;
}
...
boolean PWM_SetDutyCycle(TPWM Index, uint32_t Duty)
{
    if (Index >= PWM_CHANNELS) return false;
    else if (Index == PWM_CHANNEL2)
    {
        if (Duty < 50) PWMINFO[Index].Threshold = (0 << 0);
        else if (Duty < 100) PWMINFO[Index].Threshold = (1 << 0);
        else PWMINFO[Index].Threshold = (2 << 0);
    }
    else
    {
        Duty = ({ typeof(Duty) _a = Duty; typeof(100) _b = 100; (_a < _b) ? _a : _b; });
        if (PWMINFO[Index].PWM->Count)
            PWMINFO[Index].Threshold = ((((Duty * (PWMINFO[Index].PWM->Count - 1)) / 100) & 0x1FFF) << 0);
