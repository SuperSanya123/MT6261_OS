# 1 "Source\\Bootloader\\DZ09_boot.c"
# 1 "C:\\Users\\user\\Desktop\\DZ09-master//"
# 1 "<built-in>"
# 1 "<command-line>"
# 1 "Source\\Bootloader\\DZ09_boot.c"
# 21 "Source\\Bootloader\\DZ09_boot.c"
...
# 22 "Source\\Bootloader\\DZ09_boot.c" 2
# 1 ".\\Source\\Lib/mtktypes.h" 1
# 45 ".\\Source\\Lib/mtktypes.h"
typedef enum
{
...
typedef struct
{
    ...
    BL_Descr m_bl_desc[8];
} BR_Layout_v1, *pBR_Layout_v1;
# 23 "Source\\Bootloader\\DZ09_boot.c" 2
# 1 "Source\\Bootloader\\sha-1.h" 1
# 24 "Source\\Bootloader\\sha-1.h"
typedef struct
{
    uint32_t H0;
    uint32_t H1;
...
# 24 "Source\\Bootloader\\DZ09_boot.c" 2

# 1 "Source\\Bootloader\\dz09_boot.h" 1
# 26 "Source\\Bootloader\\DZ09_boot.c" 2

extern uint32_t __ROMImageBase, __ROMImageLimit;

void *BL_CheckFileByDescriptor(BL_Descr File)
{
    void *ExtBLEntryPoint = 
# 31 "Source\\Bootloader\\DZ09_boot.c" 3 4
                           ((void *)0)
# 31 "Source\\Bootloader\\DZ09_boot.c"
                               ;
            ...
            (File.m_bl_boundary_dev_addr < (uint32_t)&__ROMImageLimit))
    {
        ...
        if ((((FileInfo->hdr.magic) & 0x00FFFFFF) == 0x004D4D4D) &&
                (FileInfo->hdr.type == GFH_FILE_INFO) &&
                (FileInfo->hdr.size == sizeof(FILE_INFO_v1)) &&
                (FileInfo->file_type == 0x0002) &&
                (FileInfo->flash_dev == 7) &&
                ...
                (FileInfo->load_addr + FileInfo->file_len < (uint32_t)&__ROMImageLimit))
        {
            ...
            if (FileInfo->sig_type == 0)
                ExtBLEntryPoint = (void *)(FileInfo->load_addr + FileInfo->jump_offset);
            else if ((FileInfo->sig_type == 1) &&
                     (FileInfo->sig_len == sizeof(MTK_PHASH)) &&
                     (*(uint32_t *)(FileInfo->load_addr + SizeToCheck - 4) == 0x454E4400))
            {
                CheckedHash = SHA1_ProcessData((uint8_t *)FileInfo->load_addr, SizeToCheck);
                if (CheckedHash != 
# 60 "Source\\Bootloader\\DZ09_boot.c" 3 4
                                  ((void *)0)
# 60 "Source\\Bootloader\\DZ09_boot.c"
                                      )
                {
                    pMTK_PHASH FileHash = (pMTK_PHASH)(FileInfo->load_addr + SizeToCheck);

                    if ((((uint32_t)( (((uint32_t)(CheckedHash->H0) & (uint32_t)0x000000ffUL) << 24) | (((uint32_t)(CheckedHash->H0) & (uint32_t)0x0000ff00UL) << 8) | (((uint32_t)(CheckedHash->H0) & (uint32_t)0x00ff0000UL) >> 8) | (((uint32_t)(CheckedHash->H0) & (uint32_t)0xff000000UL) >> 24))) == FileHash->H0) &&
                            (((uint32_t)( (((uint32_t)(CheckedHash->H1) & (uint32_t)0x000000ffUL) << 24) | (((uint32_t)(CheckedHash->H1) & (uint32_t)0x0000ff00UL) << 8) | (((uint32_t)(CheckedHash->H1) & (uint32_t)0x00ff0000UL) >> 8) | (((uint32_t)(CheckedHash->H1) & (uint32_t)0xff000000UL) >> 24))) == FileHash->H1) &&
                            (((uint32_t)( (((uint32_t)(CheckedHash->H2) & (uint32_t)0x000000ffUL) << 24) | (((uint32_t)(CheckedHash->H2) & (uint32_t)0x0000ff00UL) << 8) | (((uint32_t)(CheckedHash->H2) & (uint32_t)0x00ff0000UL) >> 8) | (((uint32_t)(CheckedHash->H2) & (uint32_t)0xff000000UL) >> 24))) == FileHash->H2) &&
                            (((uint32_t)( (((uint32_t)(CheckedHash->H3) & (uint32_t)0x000000ffUL) << 24) | (((uint32_t)(CheckedHash->H3) & (uint32_t)0x0000ff00UL) << 8) | (((uint32_t)(CheckedHash->H3) & (uint32_t)0x00ff0000UL) >> 8) | (((uint32_t)(CheckedHash->H3) & (uint32_t)0xff000000UL) >> 24))) == FileHash->H3) &&
                            (((uint32_t)( (((uint32_t)(CheckedHash->H4) & (uint32_t)0x000000ffUL) << 24) | (((uint32_t)(CheckedHash->H4) & (uint32_t)0x0000ff00UL) << 8) | (((uint32_t)(CheckedHash->H4) & (uint32_t)0x00ff0000UL) >> 8) | (((uint32_t)(CheckedHash->H4) & (uint32_t)0xff000000UL) >> 24))) == FileHash->H4))
                    {
                        ExtBLEntryPoint = (void *)(FileInfo->load_addr + FileInfo->jump_offset);
                    }
                }
...
int main(void)
{
    ...
    RGU_SetWDTInterval(2, true);

    DBG_Initialize();
    USC_StartCounter();
    PLL_Initialize();

    EMI_MemoryRemap(MR_FB1RB0);



    if (!strcmp(sf_header->m_identifier, "SF_BOOT") &&
            (sf_header->m_ver == 1))
    {
        ...
        if (!strcmp(BR_Layout->m_identifier, "BRLYT") &&
                (BR_Layout->m_ver == 1))
        {
            uint32_t i;
            ...
            for(i = 0; i < 8; i++)
            {
                ...
                if ((BR_Layout->m_bl_desc[i].m_bl_exist_magic == 0x42424242) &&
                        (BR_Layout->m_bl_desc[i].m_bl_dev == 7) &&
                        (BR_Layout->m_bl_desc[i].m_bl_type == 0x0002) &&
                        ((Run2ndBoot = BL_CheckFileByDescriptor(BR_Layout->m_bl_desc[i])) != 
# 117 "Source\\Bootloader\\DZ09_boot.c" 3 4
                                                                                            ((void *)0)
# 117 "Source\\Bootloader\\DZ09_boot.c"
                                                                                                ))
                {
                    Run2ndBoot();
                }
